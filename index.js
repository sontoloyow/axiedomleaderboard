...
function saveWinners() {
  try {
    fs.writeFileSync(WINNERS_FILE, JSON.stringify(
      { savedAt: new Date().toISOString(), log: winnerLog.slice(0,500), tally: winnerTally, lastTs: lastWinnerTs }, null, 2));
...
function buildPage(weekVal) {
  ...
  // Build rows JSON for client-side render & sort
  const rows = (players||[]).map(p => {
    ...
    return {
      ...
      treasure: p.treasure, marbles: p.marbles, runCount: p.runCount,
      keys: p.totalKeysSpent||p.keys||0,
      keysDiff: p.keysDiff !== undefined ? p.keysDiff : null,
...
<div class="panel">
  ...
  <div class="week-bar">
    ...
    <select class="week-select" id="week-sel" onchange="location.href='/?week='+this.value">
      ${weekOptions}
    </select>
  ...
  <div class="tbl-wrap">
    ...
    <div id="lb-empty" class="empty-state" style="display:none">NO DATA FOR THIS PERIOD</div>
  </div>
</div>
${FOOTER}
